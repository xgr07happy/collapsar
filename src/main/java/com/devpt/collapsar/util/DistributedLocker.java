...
/**
 * Created by chenyong on 2015/5/13
 * <p>
...
public class DistributedLocker {
    ...
    public String lock(String name, int timeout) {
        if (timeout < 1 || timeout > MAX_LOCKER_TIME) {
            logger.error("lock: invalid timeout. It must be between 1 and 3600");
            return null;
        }
        String key = getLockerKey(name);
        if (redis.opsForValue().get(key) != null) {
            logger.debug("lock: Failed to acquire the locker '{}' because it's locked by other.", name);
            // process the expire exception case
            Long expireSeconds = redis.getExpire(key);
            if (expireSeconds == -1) {
                logger.warn("lock: reset the timeout of bad distribute locker '{}' ", key);
        ...
        try {
            ...
            if (redis.opsForValue().setIfAbsent(key, lockId)) {
                redis.expire(key, timeout, TimeUnit.SECONDS);
                logger.debug("lock: obtained distributed locker '{} ({})'", name, lockId);
                return lockId;
            } else {
                logger.debug("lock: Failed to create the locker key '{}' because it exists.", key);
                return null;
            }
        } catch (Exception e) {
            redis.delete(key);
            logger.warn("lock: exception in locking: {}", e);
            return null;
        }
    }
    ...
    public void unlock(String name, String lockerId) {
        ...
        if (value == null) {
            logger.info("unlock: the locker '{}' doesn't exists", name);
            return;
        }

        if (!value.equals(lockerId)) {
            logger.warn("unlock: invalid locker id!");
            return;
        }

        redis.delete(key);
        logger.info("unlock: the locker '{}' is unlocked", name);
    }
    ...
    public boolean update(String lockerName, int timeoutSeconds) {
        ...
        if (redis.opsForValue().get(key) != null) {
            ...
            logger.debug("update: the locker '{}' doesn't exist", lockerName);
            return false;
        }
    }
